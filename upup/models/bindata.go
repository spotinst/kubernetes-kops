...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: dns-controller
        image: k8s.gcr.io/kops/dns-controller:1.19.3
        command:
...
{{ end }}
    spec:
      ...
      containers:
      - name: kops-controller
        image: k8s.gcr.io/kops/kops-controller:1.19.3
        volumeMounts:
  ...
  template:
    ...
    spec:
      ...
      securityContext:
        fsGroup: 65534
      containers:
      - image: {{ or .Networking.Calico.Registry "docker.io" }}/calico/typha:{{ or .Networking.Calico.Version "v3.17.4" }}
        name: calico-typha
        ports:
        - containerPort: 5473
          name: calico-typha
  ...
  template:
    ...
    spec:
      ...
      initContainers:
        ...
        # upgraded to use calico-ipam.
        - name: upgrade-ipam
          image: {{ or .Networking.Calico.Registry "docker.io" }}/calico/cni:{{ or .Networking.Calico.Version "v3.17.4" }}
          command: ["/opt/cni/bin/calico-ipam", "-upgrade"]
          envFrom:
          - configMapRef:
              # Allow KUBERNETES_SERVICE_HOST and KUBERNETES_SERVICE_PORT to be overridden for eBPF mode.
        ...
        # to communicate with Felix over the Policy Sync API.
        - name: flexvol-driver
          image: {{ or .Networking.Calico.Registry "docker.io" }}/calico/pod2daemon-flexvol:{{ or .Networking.Calico.Version "v3.17.4" }}
          volumeMounts:
          - name: flexvol-driver-host
            mountPath: /host/driver
          securityContext:
      ...
      containers:
        ...
        # host.
        - name: calico-node
          image: {{ or .Networking.Calico.Registry "docker.io" }}/calico/node:{{ or .Networking.Calico.Version "v3.17.4" }}
          envFrom:
          - configMapRef:
              # Allow KUBERNETES_SERVICE_HOST and KUBERNETES_SERVICE_PORT to be overridden for eBPF mode.
              name: kubernetes-services-endpoint
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: calico-kube-controllers
          image: {{ or .Networking.Calico.Registry "docker.io" }}/calico/kube-controllers:{{ or .Networking.Calico.Version "v3.17.4" }}
          env:
            # Choose which controllers to run.
            - name: ENABLED_CONTROLLERS
              value: node
